...
export class BoundaryRing extends Container {
  ...
  update(elapsed: number, playerX: number, playerY: number, screenW: number, screenH: number) {
    ...
    this.glowGfx.stroke({ width: 35 * pulse, color: 0xFF3333, alpha: 0.15 * pulse });

    // Mid soft glow
    this.glowGfx.circle(0, 0, r);
    this.glowGfx.stroke({ width: 25 * pulse, color: 0xFF4444, alpha: 0.3 * pulse });

    // Flash on expand
    if (this.expandAnimProgress >= 0 && this.expandAnimProgress < 0.3) {
      const flashAlpha = (1 - this.expandAnimProgress / 0.3) * 0.5;
      this.glowGfx.circle(0, 0, r);
      this.glowGfx.stroke({ width: 30, color: 0xFFFFFF, alpha: flashAlpha });
    }

    // ─── Main ring ───
    this.ringGfx.clear();
    this.ringGfx.circle(0, 0, r);
    this.ringGfx.stroke({ width: 4, color: 0xFF6666, alpha: 0.8 });
